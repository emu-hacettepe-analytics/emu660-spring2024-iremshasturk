...
co2_energy <- merge(co2_emissions_to2020, combined_data_from2015to2020, by=c("Year", "month"))
cor(co2_energy$emissions, co2_energy$Total)
```

```{r}
ggplot(co2_energy, aes(x = emissions, y = Total)) +
  geom_point() +
...
annual_co2_energy <- co2_energy %>%
  ...
  summarise(
    ...
    Annual_Total_Renewable_Energy = sum(Total)
  )
```
...
-   **Correlation:** The correlation coefficient is 0.9699311, indicating a very strong positive linear relationship.

-   **Significance:** The p-value is 0.001343, which shows that the relationship is statistically significant.

-   **Confidence Interval:** The 95% confidence interval ranges from 0.7440742 to 0.9968295, suggesting that the correlation is indeed strong.
