...
Renewable Energy Production and Trends in Energy Consumption data is obtained from <a href="https://seffaflik.epias.com.tr/home">EPİAŞ Transparency Platform</a> . EPİAŞ is a vital institution for the efficient functioning of Turkey's energy markets, facilitating regulated, transparent, and reliable energy trading.
...
**Prophet** is an R package developed by Facebook and used specifically for time series forecasting.
...
-   Renewable Energy Production
    ...
    library(prophet)
    combined_data <- merge(installed_cap, annual_data_from2015to2020, by.x = "Years", by.y = "Year")
    ...
    annual_data_from2015to2020_prophet <- annual_data_from2015to2020 %>%
      select(Year, Renewable_Total) %>%
      filter(Year >= 2015)%>% 
      rename(ds = Year, y = Renewable_Total)

    # Prophet modelini eğitme
    prophet_model <- prophet(annual_data_from2015to2020_prophet)

    # Modeli eğitmek için veri aralığını belirleme
    start_date <- min(annual_data_from2015to2020$Year)
    end_date <- 2022
...
Since there is a simple linear relationship in the data, a linear regression model may be more appropriate.

-   Installed Capacity & Total Energy Production

```{r}
installed <- installed_cap %>% 
  select(Years, Total)%>% 
  rename(year = Years, total_installed = Total)

energy <- annual_data_from2015to2020 %>% 
  select(Year, Total)%>% 
  rename(year = Year, total_energy = Total)

combined_data <- merge(installed, energy)

combined_data

# 2015-2020 yılları verilerini model eğitimi için ayırma
training_data <- combined_data[combined_data$year <= 2020, ]

# Basit doğrusal regresyon modeli oluşturma
model <- lm( total_energy   ~total_installed , data = training_data)

# Modelin özetini inceleyelim
summary(model)

#"Wind", "Solar", "Geothermal", "River", "Dammed Hydro", "Biomass"

future_data <- installed %>% 
  filter(year > 2020) %>%
  select(year, total_installed) 

# Tahmin yapma
predictions <- predict(model, newdata = future_data)

# Tahmin sonuçlarını ekleyelim
future_data$Total_Energy_Production_Predicted <- predictions

# Tahmin sonuçlarını gösterme
print(future_data)

all_data <- bind_rows(
  training_data %>% mutate(Source = "Actual"),
  future_data %>% mutate(Total_Energy_Production = Total_Energy_Production_Predicted, Source = "Predicted")
)

all_data[7,'total_energy']=314683960.88
all_data[8,'total_energy']=311293161.35

# Grafiği oluşturma
p3 <- ggplot(all_data) +
  geom_point(aes(x = year, y = total_energy, color = "Actual Energy")) +
  geom_line(aes(x = year, y = total_energy, color = "Actual Energy")) +
  geom_point(aes(x = year, y = Total_Energy_Production_Predicted, color = "Predicted Energy")) +
  geom_line(aes(x = year, y = Total_Energy_Production_Predicted, color = "Predicted Energy")) +
  
  labs(title = "Actual and Predicted Energy Production",
       x = "Year",
       y = "Total Energy Production (GWh)") +
  theme_minimal()

p4 <- ggplot(all_data) +
  geom_point(aes(x = year, y = total_installed, color = "Installed cap")) +
  geom_line(aes(x = year, y = total_installed, color = "Installed cap"))+
      labs(title = "Installed Capacities",
       x = "Year",
       y = "Installed cap") +
  theme_minimal()
...
installed <- installed_cap %>% 
  ...
  rename(year = Years, renewable_installed = Renewable_Total)

energy <- annual_data_from2015to2020 %>% 
  select(Year, Renewable_Total)%>% 
  rename(year = Year, renewable_energy = Renewable_Total)
...
# Basit doğrusal regresyon modeli oluşturma
model <- lm( renewable_energy   ~renewable_installed , data = training_data)
...
future_data <- installed %>% 
  filter(year > 2020) %>%
  select(year, renewable_installed) 

# Tahmin yapma
predictions <- predict(model, newdata = future_data)

# Tahmin sonuçlarını ekleyelim
future_data$Renewable_Predicted <- predictions
...
all_data <- bind_rows(
  training_data %>% mutate(Source = "Actual"),
  future_data %>% mutate(Renewable_Energy_Production = Renewable_Predicted, Source = "Predicted")
)

all_data[7,'renewable_energy']=103928731
all_data[8,'renewable_energy']=121656648

# Grafiği oluşturma
p5 <- ggplot(all_data) +
  geom_point(aes(x = year, y = renewable_energy, color = "Actual Energy")) +
  geom_line(aes(x = year, y = renewable_energy, color = "Actual Energy")) +
  geom_point(aes(x = year, y = Renewable_Predicted, color = "Predicted Energy")) +
  geom_line(aes(x = year, y = Renewable_Predicted, color = "Predicted Energy")) +
...
p6 <- ggplot(all_data) +
  geom_point(aes(x = year, y = renewable_installed, color = "Installed cap")) +
  geom_line(aes(x = year, y = renewable_installed, color = "Installed cap"))+
      labs(title = "Installed Capacities",
       x = "Year",
       y = "Installed cap") +
  theme_minimal()
